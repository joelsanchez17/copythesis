...
def sympy_to_pytorch(expression: sp.Expr,in_args: Dict[str,List[sp.Basic]],function_name:str,condense_inputs:Optional[bool]=False,has_self:Optional[bool]=False,batched:Optional[bool]=False,tab = "    ") -> str:
    ...
    if isinstance(expression,sp.Matrix) or isinstance(expression,sp.Array):
        ...
        if batched:
            final_codeblock = codeblock_to_batched_codeblock(final_codeblock,variables_to_batch=[out_variable] + variables)
        ...
        if batched:
            final_codeblock = codeblock_to_batched_codeblock(final_codeblock,variables_to_batch=variables)
        out_var_string = ""
