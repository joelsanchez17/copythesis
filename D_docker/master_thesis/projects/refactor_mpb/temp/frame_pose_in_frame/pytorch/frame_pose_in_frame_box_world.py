...
def frame_pose_in_frame_box_world(_in0, tensor_kwargs=None):
    ...
    cte_1 = torch.tensor(2.0, **tensor_kwargs)
    cte_2 = torch.tensor(0.0, **tensor_kwargs)
    cte_3 = torch.tensor(1.0, **tensor_kwargs)

    # Intermediate terms (12)
    _tmp0 = cte_1 / (
        torch.pow(_in0[..., 10], cte_1)
        + torch.pow(_in0[..., 7], cte_1)
        + torch.pow(_in0[..., 8], cte_1)
        + torch.pow(_in0[..., 9], cte_1)
    )
    _tmp1 = -torch.pow(_in0[..., 9], cte_1) * _tmp0
    _tmp2 = -torch.pow(_in0[..., 10], cte_1) * _tmp0 + torch.tensor(
        1.0, **tensor_kwargs
    )
    _tmp3 = _in0[..., 8] * _tmp0
    _tmp4 = _in0[..., 9] * _tmp3
    _tmp5 = _in0[..., 10] * _tmp0
    _tmp6 = _in0[..., 7] * _tmp5
    _tmp7 = _in0[..., 7] * _in0[..., 9] * _tmp0
    _tmp8 = _in0[..., 8] * _tmp5
    _tmp9 = -torch.pow(_in0[..., 8], cte_1) * _tmp0
    _tmp10 = _in0[..., 7] * _tmp3
    _tmp11 = _in0[..., 9] * _tmp5
    ...
    _out = _broadcast_and_stack(
        [
            _broadcast_and_stack(
                [_tmp1 + _tmp2, _tmp4 + _tmp6, -_tmp7 + _tmp8, cte_2],
                dim=-1,
            ),
            _broadcast_and_stack(
                [_tmp4 - _tmp6, _tmp2 + _tmp9, _tmp10 + _tmp11, cte_2],
                dim=-1,
            ),
            _broadcast_and_stack(
                [
