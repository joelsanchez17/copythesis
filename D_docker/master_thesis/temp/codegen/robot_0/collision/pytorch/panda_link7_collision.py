# -----------------------------------------------------------------------------
# This file was autogenerated by symforce from template:
#     function/FUNCTION.py.jinja
# Do NOT modify by hand.
# -----------------------------------------------------------------------------

# pylint: disable=too-many-locals,too-many-lines,too-many-statements,unused-argument

import math  # pylint: disable=unused-import
import typing as T

import torch


class TensorKwargs(T.TypedDict):
    """
    TypedDict representing args that will be passed to any torch.tensor calls
    """

    device: torch.device
    dtype: torch.dtype


def _broadcast_and_stack(tensors, dim=-1):
    # type: (T.List[torch.Tensor], int) -> torch.Tensor
    """
    broadcast tensors to common shape then stack along new dimension
    """

    broadcast_shape = torch.broadcast_shapes(*(x.size() for x in tensors))
    broadcast_tensors = [x.broadcast_to(broadcast_shape) for x in tensors]

    return torch.stack(broadcast_tensors, dim=dim)


def panda_link7_collision(_in0, tensor_kwargs=None):
    # type: (torch.Tensor, TensorKwargs) -> torch.Tensor
    """
    This function was autogenerated. Do not modify by hand.

    Args:
        _in0: Matrix91

    Outputs:
        out: Matrix23

    The types for inputs and outputs above are the shapes of the symbolic matrices this function was
    generated from.  Tensors for both inputs and outputs are expected to have dimensions as follows:

    - (..., N, M) for sf.Matrix types of shape (N, M) where N != 1 and M != 1
    - (..., N) for sf.Matrix types of shape (N, M) where N != 1 and M == 1 (i.e. column vectors)
    - (..., M) for sf.Matrix types of shape (N, M) where N == 1 and M != 1 (i.e. row vectors)
    - (...) for sf.Matrix types of shape (1, 1) and sf.Scalar types

    Where `...` is an arbitrary number of batch dimensions that can be present on the front of input
    tensors.  The outputs will have the same batch dimensions as the inputs.

    The tensor_kwargs argument can be used to set the device and dtype to use.  If not provided, it will
    default to the device and dtype of the first input tensor, or the empty dict if there are no inputs.
    """

    # Total ops: 161

    # Deduce expected tensor device and dtype if not provided
    if tensor_kwargs is None:
        tensor_kwargs = {"device": _in0.device, "dtype": _in0.dtype}

    # Input arrays

    cte_1 = torch.tensor(0.535, **tensor_kwargs)
    cte_2 = torch.tensor(0.358, **tensor_kwargs)
    cte_3 = torch.tensor(0.0825, **tensor_kwargs)
    cte_4 = torch.tensor(1.0, **tensor_kwargs)
    cte_5 = torch.tensor(0.088, **tensor_kwargs)
    cte_6 = torch.tensor(0.06, **tensor_kwargs)
    cte_7 = torch.tensor(0.01, **tensor_kwargs)
    cte_8 = torch.tensor(0.384, **tensor_kwargs)
    cte_9 = torch.tensor(0.316, **tensor_kwargs)

    # Intermediate terms (54)
    _tmp0 = torch.cos(_in0[..., 3])
    _tmp1 = torch.sin(_in0[..., 0])
    _tmp2 = torch.sin(_in0[..., 2])
    _tmp3 = cte_4 * _tmp2
    _tmp4 = _tmp1 * _tmp3
    _tmp5 = torch.cos(_in0[..., 1])
    _tmp6 = torch.cos(_in0[..., 0])
    _tmp7 = torch.cos(_in0[..., 2])
    _tmp8 = cte_4 * _tmp7
    _tmp9 = _tmp6 * _tmp8
    _tmp10 = _tmp4 - _tmp5 * _tmp9
    _tmp11 = cte_4 * _tmp10
    _tmp12 = torch.sin(_in0[..., 1])
    _tmp13 = torch.sin(_in0[..., 3])
    _tmp14 = _tmp12 * _tmp13
    _tmp15 = cte_4 * _tmp14
    _tmp16 = cte_4 * torch.cos(_in0[..., 4])
    _tmp17 = _tmp1 * _tmp8
    _tmp18 = _tmp3 * _tmp6
    _tmp19 = torch.sin(_in0[..., 4])
    _tmp20 = cte_4 * _tmp19
    _tmp21 = _tmp16 * (_tmp0 * _tmp11 - _tmp15 * _tmp6) - _tmp20 * (-_tmp17 - _tmp18 * _tmp5)
    _tmp22 = torch.sin(_in0[..., 5])
    _tmp23 = cte_6 * _tmp22
    _tmp24 = cte_4 * _tmp0
    _tmp25 = _tmp12 * _tmp24
    _tmp26 = -_tmp11 * _tmp13 - _tmp25 * _tmp6
    _tmp27 = torch.cos(_in0[..., 5])
    _tmp28 = cte_6 * _tmp27
    _tmp29 = cte_8 * _tmp13
    _tmp30 = cte_3 * _tmp0
    _tmp31 = cte_5 * _tmp27
    _tmp32 = cte_9 * _tmp12
    _tmp33 = cte_3 * _tmp2
    _tmp34 = _tmp5 * _tmp7
    _tmp35 = cte_3 * _tmp6
    _tmp36 = cte_8 * _tmp0 * _tmp12
    _tmp37 = cte_5 * _tmp22
    _tmp38 = (
        _tmp1 * _tmp33
        - _tmp10 * _tmp29
        - _tmp10 * _tmp30
        + _tmp14 * _tmp35
        + _tmp21 * _tmp31
        + _tmp26 * _tmp37
        - _tmp32 * _tmp6
        - _tmp34 * _tmp35
        - _tmp36 * _tmp6
        + cte_1
    )
    _tmp39 = cte_7 * _tmp22
    _tmp40 = cte_7 * _tmp27
    _tmp41 = -_tmp17 * _tmp5 - _tmp18
    _tmp42 = cte_4 * _tmp41
    _tmp43 = -_tmp1 * _tmp25 - _tmp13 * _tmp42
    _tmp44 = _tmp16 * (_tmp0 * _tmp42 - _tmp1 * _tmp15) - _tmp20 * (-_tmp4 * _tmp5 + _tmp9)
    _tmp45 = _tmp22 * _tmp44
    _tmp46 = cte_3 * _tmp1
    _tmp47 = (
        -_tmp1 * _tmp32
        - _tmp1 * _tmp36
        + _tmp14 * _tmp46
        - _tmp29 * _tmp41
        - _tmp30 * _tmp41
        + _tmp31 * _tmp44
        - _tmp33 * _tmp6
        - _tmp34 * _tmp46
        + _tmp37 * _tmp43
    )
    _tmp48 = _tmp13 * _tmp5
    _tmp49 = _tmp12 * _tmp7
    _tmp50 = _tmp12 * _tmp19 * _tmp3 + _tmp16 * (
        -_tmp24 * _tmp49 + cte_4 * _tmp48
    )
    _tmp51 = _tmp0 * _tmp5
    _tmp52 = (
        cte_4 * _tmp13 * _tmp49
        + cte_4 * _tmp51
    )
    _tmp53 = (
        _tmp29 * _tmp49
        + _tmp30 * _tmp49
        + _tmp31 * _tmp50
        + _tmp37 * _tmp52
        - cte_3 * _tmp48
        - cte_3 * _tmp49
        + cte_9 * _tmp5
        + cte_8 * _tmp51
        + cte_2
    )

    # Output terms
    _out = _broadcast_and_stack(
        [
            _broadcast_and_stack(
                [
                    _tmp21 * _tmp23 - _tmp26 * _tmp28 + _tmp38,
                    _tmp21 * _tmp39 - _tmp26 * _tmp40 + _tmp38,
                ],
                dim=-1,
            ),
            _broadcast_and_stack(
                [
                    -_tmp28 * _tmp43 + cte_6 * _tmp45 + _tmp47,
                    -_tmp40 * _tmp43 + cte_7 * _tmp45 + _tmp47,
                ],
                dim=-1,
            ),
            _broadcast_and_stack(
                [
                    _tmp23 * _tmp50 - _tmp28 * _tmp52 + _tmp53,
                    _tmp39 * _tmp50 - _tmp40 * _tmp52 + _tmp53,
                ],
                dim=-1,
            ),
        ],
        dim=-1,
    )

    return _out
