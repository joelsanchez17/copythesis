...
docker not finding gpu drivers:
install nvdia cuda toolkit or sth
sudo nvidia-ctk runtime configure --runtime=docker
sudo systemctl restart docker
for RuntimeError: Drake could not initialize EGL by loading libEGL.so.1:

sudo apt install -y mesa-utils

jupyter not finding kernels:

install older jupyeter extenion vresion