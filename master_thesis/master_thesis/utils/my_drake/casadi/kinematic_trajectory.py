...
class KinematicTrajectoryOptimization:
    ...
    def __init__(self, casadi_plant: CasadiMultiBodyPlantWrapper, number_of_control_points: int, order: int,cse = True) -> None:
        self.opti = ca.Opti()
    ...
    def create_parameter(self, num_rows, num_columns, name_parameter: str = None) -> ca.MX:
        ...
        self.solver = None
        return p
    def create_variable(self, num_rows, num_columns, name_variable: str = None, lbx:Union[float,Iterable] = None,ubx:Union[float,Iterable] = None) -> ca.MX:
    ...
    # def set_position_
    def add_forward_kinematic_translation_constraint(self, s, frame_1, point_1, frame_2, point_2, lower_bound: Iterable = None, upper_bound: Iterable = None):
        """
        ...
        # frame_2_in_frame_1 = self.casadi_plant.calc_frame_pose_in_frame(position_at_t.nz,frame_2,frame_1,clean_small_coeffs=1e-6)

        # expr = ca.cse(frame_2_in_frame_1[0:3, 3] + frame_2_in_frame_1[0:3, 0:3] @ point_2) - point_1
        # this one is faster
        world_frame = self.casadi_plant.plant.world_frame()
        frame_pose_1 = self.casadi_plant.calc_frame_pose_in_frame(position_at_t.nz,frame_1,world_frame,clean_small_coeffs=1e-6)
        frame_pose_2 = self.casadi_plant.calc_frame_pose_in_frame(position_at_t.nz,frame_2,world_frame,clean_small_coeffs=1e-6)
    ...
    def add_forward_kinematic_orientation_constraint(self, s, frame_name_1, rotation_1, frame_name_2, rotation_2, theta_bound):
        ...
        """
        position_at_t = self.spline.evaluate(s)
        # frame_pose_1 = self.casadi_plant.calc_frame_pose_in_world(
        #     frame_name_1, position_at_t)
        # frame_pose_2 = self.casadi_plant.calc_frame_pose_in_world(
        #     frame_name_2, position_at_t)

        # orientation_1 = frame_pose_1[0:3, 0:3] @ rotation_1
        # orientation_2 = frame_pose_2[0:3, 0:3] @ rotation_2

        # trace_R1_R2 = ca.trace(orientation_1.T @ orientation_2)
        
        # this one is faster
        frame_2_in_1 = self.casadi_plant.calc_frame_pose_in_frame(position_at_t.nz,frame_name_2,frame_name_1,clean_small_coeffs=1e-6)
        # rotation_1.T @ frame_pose_1[0:3, 0:3].T @ frame_pose_2[0:3, 0:3] @ rotation_2
    ...
    def add_plant_velocity_bounds(self, lower_bound: Union[Iterable, ca.DM, ca.SX, ca.MX] = None, upper_bound: Union[Iterable, ca.DM, ca.SX, ca.MX] = None):
        """
        ...
        for i in range(self.number_of_control_points - 1):
            for j in range(self.number_of_positions):
                self.constraints.append(self.opti.bounded(lower_bound[j]*self.duration_variable,self.derivative_spline.control_points[j, i],upper_bound[j]*self.duration_variable))

        self.solver = None

    def add_plant_position_bounds(self, lower_bound: Union[Iterable, ca.DM, ca.SX, ca.MX] = None, upper_bound: Union[Iterable, ca.DM, ca.SX, ca.MX] = None):
        """
    ...
    def add_position_error_cost_on_sample(self, s:float, reference:Iterable,weight: Union[Iterable,float] = 1.0):
        """
    ...
    def add_position_error_cost_on_control_points(self, reference,weight: Union[float,int,Iterable] = 1.0):
        if isinstance(weight,(float,int)):
            weight = [weight]*self.number_of_positions
            weight = ca.vertcat(*weight)
    ...
    # def compile(self, solver_options:Dict=None):
    #     if solver_options is None:
    #         solver_options = {}
    #     import subprocess
