...
class GroupWorker:
    ...
    def compute_group(
        ...
        if num_exploitation_samples > 0:
            ...
            exploitation_W = torch.zeros(num_exploitation_samples, Y_group.shape[1], dtype=self.dtype, device=Y_group.device)
            # exploitation_H = exploitation_K @ exploitation_W
            exploitation_H = torch.zeros(num_exploitation_samples, Y_group.shape[1], dtype=self.dtype, device=Y_group.device)
            # plt.figure()
            # plt.title(f"Group {group_i}, margin before and after sv calculation")
            # print(f"{self.robot_index}_{group_i} partial time {time.perf_counter()-t_start}s")
            t0 = time.perf_counter()
            ...
            support_vector_indices = support_vector_indices.reshape(-1)
            support_vector_mask = torch.zeros(
                Y_group.shape[0], dtype=torch.bool, device=self.device
            )
            support_vector_mask[support_vector_indices] = True
            support_vector_mask[-num_exploration_samples:] = True
            masks.append(support_vector_mask)
            Y_group = Y_group[support_vector_mask]
            W_group = torch.zeros(
            ...
            K_group = K_group[support_vector_mask][:, support_vector_mask]
        ...
        # print(
        #     f"{self.robot_index}_{group_i} isclassification b4 exploration training",
        #     torch.argwhere((Y_group * H_group) < 0).shape,
        # )
        # t0 = time.perf_counter()
        ...
        # print(f"{self.robot_index}_{group_i} time 2nd pass {worker_index}: {time.perf_counter()-t0}s")
    ...
    def work(self):
        ...
        def global_trajectory_callback(msg):
            ...
            if self.index == 1:
                ...
                trajectory_points = torch.as_tensor(bspline.fast_batch_evaluate(np.linspace(0,1,20)),dtype = self.dtype,device = self.device)
            elif self.index == 2:
        ...
        if self.compile:
            with torch.no_grad():
                ...
                obstacle_points_: torch.Tensor = torch.randn(100, 3, device=self.device,dtype = self.dtype)
                obstacle_radii_: torch.Tensor = torch.randn(100, 1, device=self.device,dtype = self.dtype)
                for group_name in self.collision_model.groups:
                    for kernel_name in ['polyharmonic','rational_quadratic']:
                        kernel_modules[group_name][kernel_name] = torch.compile(kernel_modules[group_name][kernel_name],dynamic=True)
                        kernel_modules[group_name][kernel_name](configuration_,alpha_,length_scale_)
        ...
        while True:
            ...
            if trajectory_points is not None:
                ...
                self.X_exploration_buffer[exploration_samples.shape[0]: exploration_samples.shape[0]+plan_exploration_samples.shape[0]] = plan_exploration_samples
                ...
                for group_i, (group_name, group_worker) in enumerate(self.group_workers.items()):
                    ...
                    Y_total = (
                        is_group_in_collision(
                            group_name, all_samples, obstacle_points, obstacle_radii
                        )
                        * 2.
                        - 1.
                    ).reshape(-1, 1)
                    ...
                    # Y_exploitation = (
                    #     is_group_in_collision(
                    #         group_name, exploitation_samples, obstacle_points, obstacle_radii
                    #     )
                    #     * 2
                    #     - 1
                    # )
