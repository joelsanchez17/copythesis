...
def make_system(yaml_path, meshcat, CODEGEN_FOLDER, camera = False):
    ...
    for i in range(0, 2):
        ...
        # )
        robot_opt_info.collision_model = None
        robot_opt_info.vision_model = None
...
class OptimizationData:
    ...
    parameter_shapes: Dict[str, np.ndarray] = field(default_factory=Dict)
    variable_shapes: Dict[str, np.ndarray] = field(default_factory=Dict)
    initial_guess: Dict[str, np.ndarray] = field(default_factory=Dict)
    result: Dict[str, np.ndarray] = field(default_factory=Dict)
    variable_slices: Dict[str, slice] = field(default_factory=Dict)
    parameter_slices: Dict[str, slice] = field(default_factory=Dict)
    constraint_slices: Dict[str, slice] = field(default_factory=Dict)
    constraint_subnames: Dict[str, List[str]] = field(default_factory=Dict)
    x0: np.ndarray = None
    p: np.ndarray = None
    lbx: np.ndarray = None
    ubx: np.ndarray = None
