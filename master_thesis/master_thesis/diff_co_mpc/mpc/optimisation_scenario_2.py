...
class OptimizationInfo:
    ...
    def setup_optimization(
        ...
        self.gaze_options = gaze_options
        ...
        self.end_position_slack_cost_weight = self.parameter(
            "end_position_slack_cost_weight", 4, 1
        )
        ...
        if self.diff_co_options:
            ...
            self.slack_lbg = self.parameter("slack_lbg", 1, 1)
        ...
        print(self.named_variables)
        print(self.decision_variables.shape)
    ...
    def make_lbx_ubx_function(self):
        ...
        lower_limits = {
            ...
            "duration": self.t_min,
            'end_position_slack': ca.DM([-1,-1,-1]),
            'end_angle_bounds_slack': ca.DM(0.),
        ...
        for robot_name in self.svm_weights:
            ...
            for group_name in robot_slack:
                lower_limits |= {f"{robot_name}_{group_name}_svm_slack": self.slack_lbg}
        upper_limits = {
            "robot_1_control_points": ca.repmat(
            ...
            "duration": self.t_max,
            'end_position_slack': ca.DM([1,1,1]),
            'end_angle_bounds_slack': ca.DM(3.14),
        }
        for robot_name in self.svm_weights:
            robot_slack = self.svm_slack[robot_name]
            for group_name in robot_slack:
                upper_limits |= {f"{robot_name}_{group_name}_svm_slack": self.slack_ubg}
        start = 0
    ...
    def get_object_pose_constraint_function_2(
        ...
        g_orientation, lbg_orientation, ubg_orientation = (
            KinematicOptimization.fwd_kin_orientation_constraint(
                rotation_MX, ca.DM.eye(3), EE_frame_pose, angle_upper_bound + end_angle_bounds_slack
            )
        )
        g = ca.cse(veccat(g_translation, g_orientation))
        lbg = veccat(lbg_translation, lbg_orientation)
    ...
    def get_inner_samples_function(self, samples, lam_g_shape=None):
        ...
        for i,s in enumerate(samples):
            ...
            for robot_name in self.svm_weights:
                ...
                for group_name in robot_slack:
                    i = np.nonzero(np.linspace(0,1,10) == s)[0]
                    slack = self.svm_slack[robot_name][group_name][i]
    ...
    def make_sample_constraints(
        ...
        if not self.diff_co_options:
            ...
            # ]
                    ...
                    mx_out[0] += score_pol 
                    # mx_out[0] = ca.fmin(ca.fmax(-2.,mx_out[0]), 2.) 
    ...
    def contraints_on_samples(
        ...
        if name == 'sample_constraint':
            ubg['ubg'][0:8] = ca.inf
            lbg['lbg'][0:8] = -ca.inf 
            print('constraint_samples',ubg,lbg)
